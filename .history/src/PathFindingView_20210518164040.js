  ...
  clearGrid(){
    ...
    for(let i = 0; i < boardHeight; i++){
      for(let j = 0; j < boardWidth; j++){
        if(grid[i][j].wall || grid[i][j].visited || grid[i][j].visitedRealTime || grid[i][j].path)
        {
          grid[i][j].wall = false
          grid[i][j].visited = false
          grid[i][j].path = false
          grid[i][j].visitedRealTime
          this.changeStateFunction(grid[i][j],'Node')
        }
      }
    }
  }
    ...
    for(let i = 0; i < boardHeight; i++){
      for(let j = 0; j < boardWidth; j++){
        grid[i][j].previous = null
          if(grid[i][j].visitedRealTime || grid[i][j].visited || grid[i][j].path && !grid[i][j].wall && !grid[i][j].start){
            grid[i][j].visited = false
            grid[i][j].path = false
            this.changeStateFunction(grid[i][j],'Node')
          }
  ...
  realTimePath(visitedArray,solutionArray){
    ...
    for(let index of visitedArray){
      if(!grid[index[0]][index[1]].start && !grid[index[0]][index[1]].end){
        ...
        this.changeStateFunction(grid[index[0]][index[1]],'Node node-realtime')
      }
    }
    for(let index of solutionArray){
      grid[index[0]][index[1]].visited = false
      grid[index[0]][index[1]].path = false
      this.changeStateFunction(grid[index[0]][index[1]],'Node node-realtimePath')
    }
