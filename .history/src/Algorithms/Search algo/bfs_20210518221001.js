    ...
    while (startNeighbors.length && endNeighbors.length) {
        ...
        if (current.end) {
            ...
            if (!current.wall || (current.start && current.wall)) {
                ...
                if (current.top != null && !visited.has(grid[current.top][current.col].no) && !startNeighbors.has(grid[current.top][current.col].no)) {
                    grid[current.top][current.col].previous = current
        ...
        if (endNode.end) {
            ...
            if (!endNode.wall || (endNode.start && endNode.wall)) {
                ...
                startCache.add(endNode.no)
                visitedPath.push([endNode.row, endNode.col])

                if (endNode.top != null && !visited.has(grid[endNode.top][endNode.col].no) && !startNeighbors.has(grid[endNode.top][endNode.col].no)) {
                    grid[endNode.top][endNode.col].previous = endNode
                    startCache.add(grid[endNode.top][endNode.col].no)
                    startNeighbors.push(grid[endNode.top][endNode.col])
                }
                if (endNode.bottom != null && !visited.has(grid[endNode.bottom][endNode.col].no) && !startCache.has(grid[endNode.bottom][endNode.col].no)) {
                    startCache.add(grid[endNode.bottom][endNode.col].no)
                    grid[endNode.bottom][endNode.col].previous = endNode
                    startNeighbors.push(grid[endNode.bottom][endNode.col])
                }
                if (endNode.left != null && !visited.has(grid[endNode.row][endNode.left].no) && !startCache.has(grid[endNode.row][endNode.left].no)) {
                    startCache.add(grid[endNode.row][endNode.left].no)
                    grid[endNode.row][endNode.left].previous = endNode
                    startNeighbors.push(grid[endNode.row][endNode.left])
                }
                if (endNode.right != null && !visited.has(grid[endNode.row][endNode.right].no) && !startCache.has(grid[endNode.row][endNode.right].no)) {
                    startCache.add(grid[endNode.row][endNode.right].no)
