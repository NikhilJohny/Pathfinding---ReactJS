...
export default class Algorithms extends Component {
    ...
    recursiveDivisionMaze(grid, startNode, finishNode) {
        ...
        let vertical = this.range(grid[0].length);
        let horizontal = this.range(grid.length);
        let walls = [];
        this.getRecursiveWalls(vertical, horizontal, grid, startNode, finishNode);
        return walls;
      }
      ...
      //dir === 1 => Vertical
       ...
       getRecursiveWalls(vertical, horizontal, grid, startNode, finishNode) {
        ...
        if (dir === 0) {
          this.addWall(grid,dir, num, vertical, horizontal, startNode, finishNode);
      ...
      //dir === 1 => Vertical
       ...
       addWall(grid,dir, num, vertical, horizontal, startNode, finishNode) {
        ...
        if (dir === 0) {
          ...
          for (let temp of horizontal) {
            ...
            this.props.change(grid[temp][num],'Node node-wall')
          }
        } else {
          ...
          for (let temp of vertical) {
            ...
            this.props.change(grid[num][temp],'Node node-wall')
          }
        }
        if (!isStartFinish) {
          tempWalls.splice(this.randomNumber(tempWalls.length), 1);
        }
      }
    ...
    render() {
        if (this.props.paused) {
            ...
            this.recursiveDivisionMaze(clone, clone[0][0],clone[clone.length - 1][clone[0].length - 1])
        }
        return ( 
            <div>hi</div>
        )
