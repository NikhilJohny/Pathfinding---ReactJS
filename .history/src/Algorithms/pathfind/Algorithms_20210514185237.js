...
export default class Algorithms extends Component {
    ...
    async recursiveMaze(grid,start){
        ...
        while(pathStack.length){
            ...
            if(neighbors.length){
                let randomIndex = Random() % neighbors.length
                for(let i = 0; i < neighbors.length; i++){
                    grid[neighbors[i].row][neighbors[i].col].wall = true
                    this.props.change(neighbors[i],'Node node-wall')
                    if(randomIndex != i)
