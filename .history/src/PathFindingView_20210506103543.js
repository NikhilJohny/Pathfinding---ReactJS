...
class PathFindingView extends Component{
    ...
    mouseOverFunction(rows,cols){
        const grid = this.state.board;
        if(this.state.currentGrid !== grid[rows][cols]){
            if(!this.state.mousePressed){
                return;
            }
            else if(this.state.mousePressed && this.currentGrid !== grid[rows][cols]){
                console.log(rows + " " + cols)
                console.log(this.state.currentGrid.row + " " + this.state.currentGrid.col)
                grid[rows][cols].wall = !grid[rows][cols].wall
                this.setState({
                    board : grid
                })
