...
class PathFindingView extends Component {
  ...
  mouseEnterFunction(rows, cols) {
    ...
    if (!this.state.mousePressed || this.state.currentGrid === grid[rows][cols])
      return;
    if(this.state.currentGrid.start || this.state.currentGrid.end && grid[rows][cols] != this.state.currentGrid && !grid[rows][cols].start || !grid[rows][cols].end){
      if(this.state.currentGrid.start){
